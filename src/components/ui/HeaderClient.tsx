...
export function HeaderClient({ logoUrl, siteName }: HeaderClientProps) {
    ...
    <header className="bg-header border-b border-border text-foreground px-4 py-3 flex items-center justify-between sticky top-0 z-30">
      <div className="flex items-center gap-4">
        ...
        <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          {/* Logo: use uploaded logo if available, otherwise fallback to GitHub icon */}
          {logoUrl ? (
          ...
          {/* Site name from DB */}
          <span className="font-semibold hidden sm:inline-block">{siteName}</span>
        </Link>
